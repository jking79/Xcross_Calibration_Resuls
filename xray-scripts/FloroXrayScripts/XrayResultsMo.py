...
def get_gpeaks(h,lrange=[0,190],sigma=6,opt="goff",thres=5,niter=1000):
    s = TSpectrum(niter,1)
...
def FitPeaks(rootfile,histo,material,nrocs,output):
	...
	elif material == "Sn" and len(peaks) == 3:
	    gaus1 = TF1("gaus1","gaus",peaks[1][0]-15, peaks[1][0]+15)
	    gaus2 = TF1("gaus2","gaus",peaks[2][0]-20,peaks[2][0]+20)
	elif material == "Sn" and len(peaks) == 2:
...
def PlotSameNStats(arrayfithisto1, arrayfithisto2, nrocs, output): #need to check the arguments (root files) 
    for i in range(0,int(nrocs)-1):
        ...
        textmu1.DrawText(0.7,0.3,"Cu.Mean():"+ str(mu1))
        textsigma1 = TLatex()
        textsigma1.SetNDC()
        textsigma1.SetTextColor(kBlue)
        textsigma1.SetTextSize(0.03)
        textsigma1.DrawText(0.7,0.2,"Cu.Sigma():"+ str(sigma1))
        c1.Update()
        ...
        textmu2.DrawText(0.7,0.5,"Sn.Mean():"+ str(mu2))
        textsigma2 = TLatex()
        textsigma2.SetNDC()
        textsigma2.SetTextColor(kMagenta)
        textsigma2.SetTextSize(0.03)
        textsigma2.DrawText(0.7,0.4,"Sn.Sigma():"+ str(sigma2))
        c1.Update()
        #gStyle.SetOptStat(1101)
...
#                f.close()
        ...
        n_ag = len(mu_ag)
        n_sn = len(mu_sn)
        n_all = n_cu + n_mo + n_sn
        sigma_y = [0]*n_all
        mus = mu_cu + mu_mo + mu_sn
	...
	textslope = TLatex()
        ...
        textslope.DrawLatex(0.3,0.7,"e^{-}/Vcal:"+ '{:.4}'.format(1./slope) + " +/- " + '{:.4}'.format((slope_err)/pow(slope,2)))
        c1.Update()
...
PlotSameNStats(Arraytgt1,Arraytgt2,nrocs,output)
